...
fts_raw <- read_csv("data/2025/TCRMP_FishTransects_2025_webexport.csv") %>% 
   mutate(Total = rowSums(select(.,X0to5:Xgt150), na.rm=T))
frs_raw <- read_csv("data/2025/TCRMP_FishRovers_2025_webexport.csv",
                    col_types = cols(`Date completed`=col_date(format="%m/%d/%Y")))
das_raw <- read_csv("data/2025/TCRMP_Diadema_2025_webexport.csv")
...
fts_abund <- fts_raw %>% 
   ...
   mutate(flag = as.factor(ifelse(Total > MaxNum, "yes","no")))
...
tabund_flag <- fts_abund %>% filter(flag == "yes") %>%
   select(`Site name`,Rep,Name,
          `Scientific name`,`Common name`,Total, MaxNum) %>% 
   mutate(Diff = Total - MaxNum) %>% 
...
frs_abund <- frs_raw %>% 
   ...
   mutate(MinAbund = replace_values(`Abundance index`, 
                                1 ~ 1, 2 ~ 2, 3 ~ 10, 4 ~ 100, 5 ~ 1000),
          MaxAbund = replace_values(`Abundance index`, 
...
fts_size  <- fts_raw %>% 
   ...
   mutate(BinMin = recode_values(SizeBin, "0-5"~0,"6-10"~6,"11-20"~11,
          ...
          BinMax = recode_values(SizeBin, "0-5"~5,"6-10"~10,"11-20"~20,
   ...
   mutate(flag = as.factor(ifelse(BinMin > MaxSize | BinMax < MinSize, "yes","no")))
# Calculate percent flagged
persizeflag <- round(summary(fts_size$flag)[2] / nrow(fts_size) *100, 1) 
# Make table of flags
size_flag <- fts_size %>% filter(flag == "yes") %>%
   select(`Site name`,Rep,Name,
          `Common name`,SizeBin, Count, MinSize, MaxSize)
```
...
size_flag %>%
   group_by(`Common name`,SizeBin,MinSize,MaxSize) %>%
...
There were `r unsized` Diadema observed but not sized. These will be added into counts with a size of NA.
