...
donefw24 <- logs_raw %>% 
   ...
   select(`Site name`, `Type name`, Rep) %>% 
   unique()
perdonefw24 <- round(nrow(donefw24) / (33*6) * 100)
# % done w entry
done24 <- done %>% 
   filter(`Type name`=="coral health" &
             `Date completed` > as.Date("2023-12-31")) %>% 
...
Here is what each person has left to proof (just fish for now):

```{r, echo = FALSE}
done %>% 
   filter((`Type name`=="fish transect" | `Type name`=="fish rover") &
             !Proofed) %>% 
   select(Name, `Site name`, `Type name`, Rep) %>% 
   group_by(Name, `Type name`) %>% 
   summarize(n=length(Rep), .groups="drop") %>% 
