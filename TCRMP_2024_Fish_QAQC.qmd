...
fts_raw <- read_csv("data/2024/FishTransects_all_2025-02-26.csv") %>% 
   mutate(Total = rowSums(select(.,X0to5:Xgt150), na.rm=T))
frs_raw <- read_csv("data/2024/FishRovers_all_2025-02-26.csv")
das_raw <- read_csv("data/2024/Diadema_all_2025-02-26.csv")
...
Did we see any sharks in transects?

```{r}
fts_raw %>% 
   left_join(select(species, `Scientific name`=SCIENTIFIC.NAME,
                    Family = FAMILY_UVI), by="Scientific name") %>% 
...
Did we see any sharks in rovers?
