...
# Read qaqc'd data for corals and algae
chs_raw <- read_csv("data/2025/TCRMP_CoralHealth_2025_webexport.csv") %>% 
   replace_na(list(`Old mortality`=0, `Recent mortality`=0,
                  `Bl sp`=0,`Bl p`=0, `Bl vp`=0, `Bl bl`=0))
ahs_raw <- read_csv("data/2025/TCRMP_AlgaeHeights_2025_webexport.csv")

# Species master for QAQC numbers
species <- read_csv("data/corallists/coral_codes_2025.csv") %>% 
...
Coral health 2025 has `r numcorals` corals.
...
# Join to master list to set flags
   left_join(species, by=c("Coral full name"="full_name")) %>% 
   # If length or height pass the 95%, flag it.
   mutate(flagL = ifelse(`Length cm` > max_diam, "yes","no"),
          flagH = ifelse(`Height cm` > max_height,"yes","no"))
...
size_flag <- coral_size %>% filter(flagL == "yes" | flagH =="yes") %>%
   select(`Site name`,Rep,Name,
          ...
          `Length cm`,max_diam,`Height cm`,max_height)
```
...
size_flag %>%
   ...
   group_by(`Coral full name`,max_diam,max_height) %>%
   summarize(FlagsL = sum(flagL),
             FlagsH = sum(flagH), .groups="drop") %>%
   mutate(Flags = FlagsL+FlagsH) %>% 
   select(`Coral full name`, Flags, FlagsL, max_diam, FlagsH,max_height) %>% 
...
```{r, fig.cap="Figure 5. Prevalence of old and new mortality by diver. Check for possible over- or under-estimating."}
